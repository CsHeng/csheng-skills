...
def render_repair_loop(contract_path: Path, contract: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            f"{alias('review')} --> {alias('classify')} : fresh full verdict",
    ...
    lines.extend(
        [
            ...
            f"  Expected convergence: {repair['expected_rounds']} rounds",
            f"  Hard limit: {repair['hard_limit']} rounds",
            "  One round repairs the complete accepted finding batch.",
            "end note",
            "@enduml",
            "",
        ]
