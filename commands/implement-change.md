...
This command is the command-surface wrapper for `coding:implement-change`.
...
- `--max-rounds <n>`: optional controller-owned implementation review/repair cap. Defaults to the hard limit `10` and must satisfy `1 <= n <= 10`; expected convergence remains `5`
...
```
- If the normalized review verdict is `needs-fixes`, classify the complete finding batch. For findings where every blocking item is `scope_class: in_scope_blocking`, enter the controller-owned repair loop instead of stopping.
- In each repair round, fix the complete accepted finding batch inside `allowed_touch_set`, run the affected and declared verification commands, save prior findings as reviewer context, and request a fresh complete review with the next round metadata.
- Expect convergence within 5 rounds, default the operational cap to 10, and allow evidence-driven convergence through that hard limit. Round 5 is not a stop condition when scope and oracle evidence are improving.
- If any blocking finding is outside local repair scope, stop with the typed `replan`, `redesign`, `needs-authority`, external-verification, or rollback state.
- If round 10 still does not pass, stop with `non-convergent` and report the remaining findings and oracle evidence.
...
bash "$RUNNER" rollback-target "<failure_kind>" "<failure_count>"
```
- If the execution verdict is `needs-fixes`, continue the controller-owned repair loop while the findings remain in scope and the hard limit is not exhausted
