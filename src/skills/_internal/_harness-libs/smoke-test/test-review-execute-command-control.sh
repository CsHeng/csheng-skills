...
main() {
  ...
  assert_contains "commands/review-code-impl.md" '--timeout <seconds>' "review-code-impl command should expose timeout control"
  assert_contains "commands/review-code-impl.md" 'args\+=\(--timeout "\$timeout_seconds"\)|Bash tool.*timeout' "review-code-impl command should apply the same timeout to Bash tool and shared runner"
  ...
  assert_contains "commands/execute-change.md" 'workspace-mode|worktree-preflight-required|git-worktrees' "execute command should perform one-time worktree preflight"
  ...
  assert_contains "commands/execute-change.md" 'task-catalog|task-ledger|current_task|completed_task_count|remaining_task_count' "execute command should materialize task progress from a task ledger"
  assert_contains "commands/execute-change.md" 'task-scoped review|coding:review-change|quick|thorough' "execute command should define task-level review cadence"
