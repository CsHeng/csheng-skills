...
usage() {
  cat <<'USAGE'
Usage:
  eval/run-eval.sh --mode <plan|design|code-impl|all> --reviewer <claude|codex> [options]

Options:
  --mode <plan|design|code-impl|all>   Review mode to evaluate. Required.
  --reviewer <claude|codex>            Reviewer CLI to use. Required.
...
case "$REVIEWER" in
  claude|codex) ;;
  *) die "invalid reviewer: $REVIEWER. Must be claude or codex" ;;
esac
