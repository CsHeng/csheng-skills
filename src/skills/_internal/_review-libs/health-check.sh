...
# Check schema file
check_schema() {
  local schema_path="$SCRIPT_DIR/schemas/reviewer-output.schema.json"

  if [[ ! -f "$schema_path" ]]; then
    error "  reviewer-output: missing"
    return 1
  fi

  if jq . "$schema_path" >/dev/null 2>&1; then
    success "  reviewer-output: valid"
    return 0
  else
    error "  reviewer-output: invalid"
    return 1
  fi
}
