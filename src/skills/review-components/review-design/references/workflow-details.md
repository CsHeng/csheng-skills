...
5. Use a command wrapper or a resolved plugin-root runner path:
   ...
   - default `--depth auto` resolves to boundary-focused design review
   - default `--max-rounds` resolves to `1` for design review
...
12. `--max-rounds` may only tighten or explicitly override the loop below the hard cap of 3; default is 1 for design review.
...
- Do not treat implementation details as design blockers unless they make the architecture boundary or downstream implementation surface unreviewable.
